...
- **Próximo passo (fluxo):** `/spec-plan` → **`/gpt-builder`** (aqui) → `/build-review`. Terminada a construção, ofereça `/build-review` (opcional) — 3 revisores independentes sobre o diff + o `.checks/<feature>.md` que esta skill já escreveu. O fiscal daqui prova a aderência à spec; o build-review é o pente-fino de qualidade por cima.
